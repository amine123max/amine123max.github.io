...
function extractResumeData() {
  ...
  if (!data.projects.length) {
    ...
    document.querySelectorAll('.featured-project-link').forEach(featuredProject => {
      const name = featuredProject.querySelector(`.featured-project-title span[data-lang="${lang}"]`)?.textContent?.trim();
      const description = featuredProject.querySelector(`.featured-project-description span[data-lang="${lang}"]`)?.textContent?.trim();
...
// Initialize
function initResumePDFGenerator() {
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initResumePDFGenerator);
} else {
